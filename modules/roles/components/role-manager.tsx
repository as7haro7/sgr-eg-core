      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-2">
        {roles.map((role) => (
          <article key={role.id} className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
            ...
            <details className="mt-4 rounded-lg bg-slate-50">
              <summary className="cursor-pointer rounded-lg px-3 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-100">
                Ver permisos ({role.permissions.length} módulos)
              </summary>
              <ul className="space-y-2 border-t border-slate-200 p-3 text-xs text-slate-600">
                {role.permissions.map((permission) => (
                  <li key={permission.moduleCode} className="flex flex-col gap-0.5 sm:flex-row sm:justify-between">
                    <span className="font-semibold">{permission.moduleName}</span>
                    <span>{permissionLabels(permission)} · alcance {permission.scope}</span>
                  </li>
                ))}
